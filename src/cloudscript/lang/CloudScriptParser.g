parser grammar CloudScriptParser;

options {
  output = AST; // Produce a tree of node
  tokenVocab = CloudScriptLexer; // Use tokens defined in our lexer.
...
literal returns [Node node]:
  ...
  | NIL               { $node = new LiteralNode(CloudScriptRuntime.getNil()); }
  | TRUE              { $node = new LiteralNode(CloudScriptRuntime.getTrue()); }
  | FALSE             { $node = new LiteralNode(CloudScriptRuntime.getFalse()); }
